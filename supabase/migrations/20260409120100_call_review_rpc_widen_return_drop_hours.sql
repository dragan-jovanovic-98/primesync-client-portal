-- Plan A follow-up: widen get_portal_calls return shape and drop p_hours.
--
-- The portal's per-location business-hours classification lives in TypeScript
-- (it joins clients + locations + assistants + closures), not in SQL. The
-- earlier RPC's `p_hours` parameter used a flat 9-17 window which is wrong.
-- We instead drop p_hours and widen the return shape so the action layer can
-- continue to hydrate each row with isBusinessHours after the RPC returns.
